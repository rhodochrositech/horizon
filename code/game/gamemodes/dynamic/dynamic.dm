...
/datum/game_mode/dynamic/Topic(href, href_list)
	...
	if (href_list["forced_extended"])
		...
		var/added_rule = tgui_input_list(
			usr,
			"What ruleset do you want to force upon the next latejoiner? This will bypass threat level and population restrictions.",
			"Rigging Latejoin",
			sortNames(init_rulesets(/datum/dynamic_ruleset/latejoin))
		)
		if (!added_rule)
			return
		...
		var/added_rule = tgui_input_list(
			...
			"Execute Ruleset",
			sortNames(init_rulesets(/datum/dynamic_ruleset/midround))
		)
		if (!added_rule)
			return
