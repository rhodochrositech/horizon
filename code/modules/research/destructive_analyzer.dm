...
/obj/machinery/rnd/destructive_analyzer/proc/user_try_decon_id(id, mob/user)
	...
	if (id && id != RESEARCH_MATERIAL_DESTROY_ID)
		...
		if(length(worths) && !length(differences))
			return FALSE
		var/choice = tgui_alert(usr, "Are you sure you want to destroy [loaded_item] to [!length(worths) ? "reveal [TN.display_name]" : "boost [TN.display_name] by [json_encode(differences)] point\s"]?", buttons = list("Proceed", "Cancel"))
		if(choice == "Cancel")
			return FALSE
