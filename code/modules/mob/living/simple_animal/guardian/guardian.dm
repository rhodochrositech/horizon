...
/mob/living/proc/guardian_reset()
	...
	if(guardians.len)
		var/mob/living/simple_animal/hostile/guardian/G = tgui_input_list(usr, "Pick the guardian you wish to reset", "Guardian Reset", sortNames(guardians))
		if(G)
...
/obj/item/guardiancreator/proc/spawn_guardian(mob/living/user, mob/dead/candidate)
	...
	if(random)
		...
		guardiantype = tgui_input_list(user, "Pick the type of [mob_name]", "[mob_name] Creation", sortList(possible_guardians))
